    ...
    def update_view_model(self, lsm_run_index: int, station_name=None, survey_name=None):
        ...
        if lsm_run_index == -1:  # No data available => Invalid index => Reset model data
            ...
            try:
                ...
                if station_name is not None:  # Filer data for station name
                    pass

            if (station_name is not None) or (survey_name is not None):
                # self._data = results_stat_df.loc[tmp_filter, :].copy(deep=True)
                pass
            else:  # No filter
    ...
    def update_view_model(self, lsm_run_index: int, station_name=None, survey_name=None):
        ...
        if lsm_run_index == -1:  # No data available => Invalid index => Reset model data
            ...
            try:
                ...
                if station_name is not None:  # Filer data for station name
                    tmp_filter = self._data['station_name'] == station_name

            if (station_name is not None) or (survey_name is not None):
                self._data = results_stat_df.loc[tmp_filter, list(self._PLOT_COLUMNS)].copy(deep=True)
            else:  # No filter
    ...
    def update_view_model(self, lsm_run_index: int, survey_name=None):
        ...
        if lsm_run_index == -1:  # No data available => Invalid index => Reset model data
            ...
            try:
                ...
                if survey_name is None:  # Filer data for survey name
                    self._data = results_drift_df.copy(deep=True)
                else:
                    # self._data = results_stat_df.loc[tmp_filter, :].copy(deep=True)
                    pass
...
# TODO: Format table views for estimation results!
# TODO: Filter results for visualization
